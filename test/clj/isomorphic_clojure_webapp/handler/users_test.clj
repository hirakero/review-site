...
(deftest handler-users-test
  (testing "get /users データがない場合は空のベクタを返す"
    (let [{:keys [status body]} (helper/http-get "/users")]
      (is (=  404 status))
      (is (=  [] (:users body)))))

  (let [{:keys [status body]} (helper/http-post "/users"
                                                {:name "Alice"
    ...
    (testing "get /users/:user-id "
      (testing "取得した内容を返す"
        (let [{:keys [status body] :as all} (helper/http-get (str "/users/" id))]
          (is (= 200 status))
          (is (= "Alice" (-> body :user :name)))))

      (testing "対象データが無ければ not found"
        (let [{:keys [status body] :as all} (helper/http-get (str "/users/00000000-0000-0000-0000-000000000000"))]
          (is (= 404 status)))))

    (testing "put /users/:user-id"
      (testing "更新した内容を返す"
        (let [{:keys [status body]} (helper/http-put (str "/users/" id) {:name "Alice Ackerman"})]
          (is (= 200 status))
          (is (= "Alice Ackerman" (-> body :user :name)))))
      (testing "対象データが無ければ not found"
        (let [{:keys [status body]} (helper/http-put "/users/00000000-0000-0000-0000-000000000000" {:name "Alice Ackerman"})]
          (is (= 404 status)))))

    (testing "get /users データの配列を返す"
      (helper/http-post "/users"
                        {:name "Bob"
    ...
    (testing "delete"
      (testing "削除に成功したらno content"
        (let [{:keys [status body]} (helper/http-delete (str "/users/" id))]
          (is (= 204 status))))
      (testing "対象データが無ければ not found"
        (let [{:keys [status body]} (helper/http-delete "/users/00000000-0000-0000-0000-000000000000")]
          (is (= 404 status)))))))