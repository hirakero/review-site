...
(deftest handler-products-test
  (testing "get /products データがない場合は空のベクタを返す"
    (let [{:keys [status body] :as all} (helper/http-get "/products")]
      (is (=  404 status))
      (is (=  [] (:products body)))))
  (let [{:keys [status body]} (helper/http-post "/products"
                                                {:name "Hammer XT"
    ...
    (testing "get /products/:product-id "
      (testing "取得した内容を返す"
        (let [{:keys [status body] :as all} (helper/http-get (str "/products/" id))]
          (is (= 200 status))
          (is (= "Hammer XT" (-> body :product :name)))))

      (testing "対象データが無ければ not found"
        (let [{:keys [status body] :as all} (helper/http-get (str "/products/00000000-0000-0000-0000-000000000000"))]
          (is (= 404 status)))))

    (testing "put /products/:product-id"
      (testing "更新した内容を返す"
        (let [{:keys [status body]} (helper/http-put (str "/products/" id) {:name "Hammer LT"})]
          (is (= 200 status))
          (is (= "Hammer LT" (-> body :product :name)))))
      (testing "対象データが無ければ not found"
        (let [{:keys [status body]} (helper/http-put "/products/00000000-0000-0000-0000-000000000000" {:name "Hammer LT"})]
          (is (= 404 status)))))

    (testing "get /products データの配列を返す"
      (helper/http-post "/products"
                        {:name "axiom ocuralis"
                         :description "ocuralis plug"})
      (let [{:keys [status body]} (helper/http-get "/products")]
        (is (= 200 status))
        (is (= 2 (-> body :products count)))))

    (testing "delete"
      (testing "削除に成功したらno content"
        (let [{:keys [status body]} (helper/http-delete (str "/products/" id))]
          (is (= 204 status))))
      (testing "対象データが無ければ not found"
        (let [{:keys [status body]} (helper/http-delete "/products/00000000-0000-0000-0000-000000000000")]
          (is (= 404 status)))))))
